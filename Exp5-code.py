...
def download_module():
    """Download module_DES.so file if not exists"""
    module_url = "https://github.com/Lab2Phys/module_DES/raw/refs/heads/main/module_DES.so"
    module_filename = "module_DES.so"
    
    if not os.path.exists(module_filename):
        print("Downloading module_DES.so...")
        try:
...
try:
    from module_DES import CircuitAnalyzer
    ...
    print(f"❌ Error importing module_DES: {e}")
